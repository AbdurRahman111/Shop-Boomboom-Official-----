...
from checkout.models import Order_Table, Order_Table_2, Customer_delivery_information, Order_Table_3
...
def deshboard_index(request):
    ...
    user_name_for_dashboard = ''
    types_of_role = ''
    Prioraty_rol = ''
    if staff_admin:
        user_name_for_dashboard = staff_admin
        types_of_role = 'Staff Admin'
        Prioraty_rol = '0'
    if staff_shop_manager:
        user_name_for_dashboard = staff_shop_manager
        types_of_role = 'Shop Manager'
        Prioraty_rol = '0'
    if staff_customer_support:
        user_name_for_dashboard = staff_customer_support
        types_of_role = 'Customer_support'
        Prioraty_rol = '0'
    if staff_upload_team:
        user_name_for_dashboard = staff_upload_team
        types_of_role = 'Upload team'
        Prioraty_rol = '0'

    get_all_revenue = 0
    get_all_revenue_Order_Table_3 = 0
    get_all_revenue_row = Order_Table.objects.all()
    get_all_revenue_row_from_Order_Table_3 = Order_Table_3.objects.all()
    for i in get_all_revenue_row:
        if i.Order_Status == 'Completed':
            get_all_revenue = get_all_revenue+ i.GrandTotal_Price
    for j in get_all_revenue_row_from_Order_Table_3:
        if j.old_order_status == 'Completed':
            get_all_revenue_Order_Table_3 = get_all_revenue_Order_Table_3 + j.old_order_total
    ...
    # k =date_today_get__lte
    # print(date_today_get)
    now_month_date = datetime.now()
    now_month = datetime.now().month
    print(now_month)

    get_month_all_revenue_row = Order_Table.objects.filter(Paid_Date__month = now_month)
    print(get_month_all_revenue_row)
    value_get_month_all_revenue_row = 0

    for i in get_month_all_revenue_row:
        value_get_month_all_revenue_row = value_get_month_all_revenue_row + i.GrandTotal_Price
    Total_revenue = get_all_revenue + get_all_revenue_Order_Table_3
    
    
    all_user_get = User.objects.all().count()
    ...
    contex = {
        ...
        'value_get_month_all_revenue_row':value_get_month_all_revenue_row,
        'all_user_get':all_user_get,
        'all_vendor_get':all_vendor_get,
        'now_month_date':now_month_date
    }

    if staff_admin or staff_shop_manager or staff_customer_support or staff_upload_team:
        return render(request, "deshboard_index.html", contex)
    else:
        return redirect('deshboard_login')
...
def dashbord_all_product(request):
    ...
    if staff_admin or staff_shop_manager or staff_upload_team:
        all_product_show = Products.objects.all().order_by('-Time')
    ...
    if staff_admin or staff_shop_manager or staff_upload_team:
        ...
        if request.method == 'POST':
            ...
            if form_product.is_valid():
                ...
                # for making product link
                site = get_current_site(request)
                print(site.domain)
                prod_link_url = str(site.domain) + '/product_details/'+ str(other_value.slug)
                messages.success(request, f'{prod_link_url}')
        ...
        count_prd = Products.objects.all().count()
        context = {'form_product': form_product, 'get_all_categories':get_all_categories, 'count_prd':count_prd}
...
def dashbord_Add_Brand(request):
    ...
    if staff_admin or staff_shop_manager or staff_upload_team:
        ...
        total_brnd = All_Brand.count()
        CONTEXT = {'All_Brand': All_Brand, 'brand_form':brand_form, 'total_brnd':total_brnd}
...
def dashbord_Categories(request):
    ...
    if staff_admin or staff_shop_manager or staff_upload_team:
        ...
        count_cat = get_all_categories.count()
        count_subcat1 = get_all_subcategories_1.count()
        count_subcat2 = get_all_subcategories_2.count()

        CONTEXT = {'form_categories': form_categories, 'get_all_categories': get_all_categories,
                   'form_subcategories': form_subcategories, 'form_subcategories_2':form_subcategories_2, 'get_all_subcategories_1': get_all_subcategories_1, 'get_all_subcategories_2':get_all_subcategories_2, 'count_cat':count_cat, 'count_subcat1':count_subcat1, 'count_subcat2':count_subcat2}
...
def filter_action(request):
    ...
    if staff_admin or staff_shop_manager or staff_upload_team:
        if request.method == "POST":
            ...
            all_product_show = Products.objects.filter(Category=get_category).filter(Stock_status=productorder_length_stock_status).order_by('-Time')
